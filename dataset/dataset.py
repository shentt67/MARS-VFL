...
class VISIONTOUCH_VFL(Dataset):
    def __init__(
        ...
        #         ProcessForce(32, "unpaired_force", tanh=True),
        #         ToTensor(),
        #         combine_modalitiesbuilder(unimodal=None, output='ee_yaw_next'),
        #     ]
        # ),
        ...
        if self.args.eval_mode == 'robustness':
            ...
            if args.perturb_type == 'missing':
                ...
                # for client in range(1, self.args.client_num):
                #     misaligned_data = misaligned(np.array([self.data[i][distribution[client]] for i in idxes]))
                # for i in range(len(idxes)):
                #     self.data[idxes[i]][distribution[self.args.misaligned_client]] = misaligned_data[i]
    ...
    def __init__(self, args, dtype) -> None:
        ...
        if args.eval_mode == 'robustness':
            ...
            if args.perturb_type == 'missing':
                ...
                # misaligned_data = misaligned(np.array([self.data[i][distribution[self.args.misaligned_client]] for i in idxes]))
                # for i in range(len(idxes)):
                #     self.data[idxes[i]][distribution[args.misaligned_client]] = misaligned_data[i]

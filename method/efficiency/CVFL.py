...
def cvfl(trainer, model_list, input, communication_cost):
        ...
        if q == 0:
            ...
            if trainer.args.dataset in ['MOSI', 'MOSEI', 'URFUNNY', 'MUSTARD']:
                for i in range(trainer.args.client_num):
                    ...
                    pred_compressed = compress(t, trainer.args.compression_ratio)
                    if isinstance(t, tuple):
                        pred_compressed = (pred_compressed, t[1])
                    local_output_list.append(pred_compressed)

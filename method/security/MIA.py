...
class custom_AE(nn.Module):
    def __init__(self, latent_dim, target_dim, args):
...
class Trainer_MIA:
    def __init__(self, device, model_list, optimizer_list, criterion, train_loader, valid_loader,
        ...
        self.args = args
        # divide parts of train data into auxiliary data
        train_data_complete = copy.deepcopy(self.train_loader.dataset)

        dataset_length = len(train_data_complete)

        train_dataset, auxiliary_dataset = torch.utils.data.random_split(train_data_complete, [
            dataset_length - int(args.auxiliary_ratio * dataset_length),
            int(args.auxiliary_ratio * dataset_length)])

        process = partial(_process_emo, args=self.args) if args.dataset in ['MOSI', 'MOSEI', 'URFUNNY', 'MUSTARD'] else partial(_process_none, args=self.args)
        self.train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True,
                                                        collate_fn=process)
        self.auxiliary_loader = torch.utils.data.DataLoader(dataset=auxiliary_dataset, batch_size=args.batch_size,
                                                            shuffle=True,
                                                            collate_fn=process)
    ...
    def train_attack(self):
        ...
        input_dim = self.model_list[self.args.attack_target_client + 1].backbone[-2].out_features
        output_dim = x_split_list[self.args.attack_target_client].shape[-1]
        self.decoder = custom_AE(input_dim, output_dim, self.args).to(self.device)
        self.optimizer_decoder = torch.optim.Adam(self.decoder.parameters(), lr=self.args.lr)
        ...
        for epoch in range(self.args.attack_train_epochs_mia):
            self.decoder.train()
            losses = []
            for step, (x_n, length, index, y, mask) in enumerate(self.auxiliary_loader):
                x = x_n
                if isinstance(x, list):
                    for i in range(len(x)):
                        x[i] = x[i].to(self.device).float()
                ...
                if self.args.dataset in ['MOSI', 'MOSEI', 'URFUNNY', 'MUSTARD']:
                    local_embedding = model_list[self.args.attack_target_client + 1](
                        [x_split_list[self.args.attack_target_client], length[self.args.attack_target_client]])
                else:
                    local_embedding = model_list[self.args.attack_target_client + 1](
                        x_split_list[self.args.attack_target_client])

                generated_feature = self.decoder(local_embedding)
                generated_feature = generated_feature[:, :x_split_list[self.args.attack_target_client].size(1), :]
                train_loss = criterion(generated_feature, x_split_list[self.args.attack_target_client])
                ...
                if step % self.args.print_steps == 0:
                    self.logger.info(
                        'Epoch: {}, {}/{}: train mse loss: {:.4f}'.format(
                            epoch + 1,
                            step + 1,
            ...
            if validate_loss < best_validate_loss:
                ...
                state = {
                    ...
                    'optimizer': self.optimizer_decoder,
                }
    ...
    def validate_attack(self, epoch):
        ...
        for step, (x_n, length, index, y, mask) in enumerate(self.valid_loader):
            ...
            loss = criterion(generated_feature, x_split_list[self.args.attack_target_client])

            losses_valid.append(loss.detach())

            loss_pf = self.lossPerFeature(x_split_list[self.args.attack_target_client], generated_feature)

            losses_valid_pf.append(loss_pf)

            loss_valid_mse = criterion(x_split_list[self.args.attack_target_client], generated_feature).item()

            losses_valid_mse.append(loss_valid_mse)

            randomguess = torch.rand_like(x_split_list[self.args.attack_target_client])
            loss_random = criterion(x_split_list[self.args.attack_target_client], randomguess).item()
    ...
    def test_attack(self, epoch):
        ...
        self.logger.info("=> loading best test checkpoint of generator '{}'".format(check_path))
        checkpoint_test = torch.load(check_path, map_location=self.device)
        decoder = self.decoder.eval()
        decoder.load_state_dict(checkpoint_test['state_dict'])
        self.logger.info("=> loaded test checkpoint '{}' (epoch {})"
                         .format(check_path, checkpoint_test['epoch']))
        losses_test = []
        losses_test_mse = []
        ...
        for step, (x_n, length, index, y, mask) in enumerate(self.test_loader):
            ...
            loss_test_mse = criterion(x_split_list[self.args.attack_target_client], generated_feature).item()
        ...
        self.logger.info(
            'Test Epoch: {}, test samples: {}, test loss: {}, test mse loss: {}, test mse loss per feature: {}, test random mse loss: {}, test random mse loss per feature: {}'.format(
                epoch + 1,
                len(self.test_loader.dataset),
                sum(losses_test) / len(losses_test), sum(losses_test_mse) / len(losses_test_mse),
                (sum(losses_test_pf) / len(losses_test_pf)).mean(), sum(losses_random) / len(losses_random),
                sum(losses_random_pf) / len(losses_random_pf)))
